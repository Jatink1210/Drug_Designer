...
class IndiGenLoader(BaseConnector):
    name = "IndiGen"
    data_dir = Path("data/external/indian/indigen")

    async def search(self, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        results = []
        if not self.data_dir.exists():
            log.warning(
                "indigen.data_dir_missing",
                path=str(self.data_dir),
                hint="Download IndiGen TSV files to this directory to enable Indian population variant search.",
            )
            return results
            ...
            try:
                with open(file_path, "r") as f:
                    ...
                    for row in reader:
                        ...
                        # Match if any substantial query token exists in ID, gene, or clinical significance
                        match = False
    ...
    def normalize(self, raw_data: Dict[str, Any]) -> VariantEntity:
        ...
        evidence = PopulationEvidence(
            ...
            notes="Found in IndiGen variant database."
        )
        ...
        variant = VariantEntity(
            ...
            indian_demographic_context="Variant present in IndiGen cohorts."
        )
        variant.provenance.append(
            self._prov(
                url=f"local://{self.data_dir}", 
                reasoning="Parsed from IndiGen local TSV files."
            ).model_dump()
        )
        return variant
