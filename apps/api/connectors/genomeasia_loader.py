...
class GenomeAsiaLoader(BaseConnector):
    name = "GenomeAsia"
    data_dir = Path("data/external/indian/genomeasia")

    async def search(self, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        results = []
        if not self.data_dir.exists():
            log.warning(
                "genomeasia.data_dir_missing",
                path=str(self.data_dir),
                hint="Download GenomeAsia TSV files to this directory to enable Indian population variant search.",
            )
            return results
    ...
    def normalize(self, raw_data: Dict[str, Any]) -> VariantEntity:
        ...
        freq = VariantFrequency(
            population="GenomeAsia 100K (South Asia)",
            allele_frequency=af,
        ...
        evidence = PopulationEvidence(
            ...
            notes="Found in GenomeAsia 100K database."
        )
        ...
        variant = VariantEntity(
            ...
            indian_demographic_context="Variant present in GenomeAsia South Asian cohorts."
        )
        variant.provenance.append(
            self._prov(
                url=f"local://{self.data_dir}", 
                reasoning="Parsed from GenomeAsia local TSV files."
            ).model_dump()
        )
        return variant
