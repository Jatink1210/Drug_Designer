"""IndiGen API connector for Indian population genomics."""
...
class IndiGenAPIConnector(BaseConnector):
    """IndiGen API connector for Indian population genomics data."""
    
    name = "IndiGen_API"
    BASE = "https://indigen.igib.res.in/api"
    cache_ttl = 86400
    rate_limit_rps = 2.0
    rate_limit_burst = 4
    http_timeout = 30.0
    ...
    async def search(self, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        for variant in data["variants"][:limit]:
            ...
            results.append({
                ...
                "name": variant_id,
                "description": f"IndiGen variant: {variant_id}",
                "variant_id": variant_id,
                "allele_freq_ind": variant.get("allele_freq"),
                "source": self.name,
                "url": "https://indigen.igib.res.in/",
                "provenance": [self._prov(
                    url="https://indigen.igib.res.in/",
                    ext_id=variant_id,
                    confidence=0.90,
                    reasoning="IndiGen Indian population genomics"
                ).to_dict()],
            })
        return results
    ...
    async def fetch_by_id(self, entity_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "name": entity_id,
            "description": f"IndiGen variant: {entity_id}",
            "variant_id": entity_id,
            "allele_freq_ind": data.get("allele_freq"),
            "source": self.name,
            "url": "https://indigen.igib.res.in/",
            "provenance": [self._prov(
                url="https://indigen.igib.res.in/",
                ext_id=entity_id,
                confidence=0.90,
                reasoning="IndiGen detailed variant data"
            ).to_dict()],
        }
