...
router = APIRouter(prefix="/api/v1/molecules", tags=["molecules"], dependencies=[Depends(get_current_user)])
...
@router.post("/score")
async def score_molecules(req: ScoreRequest, request: Request) -> Dict[str, Any]:
    """Compute physicochemical properties for a list of SMILES."""
    return build_envelope(request, [compute_physichem_rdkit(s) for s in req.smiles])


@router.post("/admet")
async def predict_admet(req: ScoreRequest, request: Request) -> Dict[str, Any]:
    """Predict ADMET properties for a list of SMILES."""
    return build_envelope(request, [_admet.predict(s) for s in req.smiles])


@router.post("/analogs")
async def generate_analogs(req: AnalogRequest, request: Request) -> Dict[str, Any]:
    ...
    return build_envelope(request, {"query_smiles": req.smiles, "method": req.method, "analogs": analogs})


@router.post("/novelty")
async def check_novelty(req: NoveltyRequest, request: Request) -> Dict[str, Any]:
    """Check novelty of a molecule against publications and patents."""
    return build_envelope(request, await _novelty.check_novelty(req.smiles))


@router.get("/iterations")
async def list_iterations(request: Request) -> Dict[str, Any]:
    """List saved design iterations."""
    return build_envelope(request, _iterations.list_iterations())
...
async def save_iteration(req: IterationSaveRequest, request: Request) -> Dict[str, Any]:
    ...
    return build_envelope(request, {"iteration_id": iteration_id, "status": "saved"})
