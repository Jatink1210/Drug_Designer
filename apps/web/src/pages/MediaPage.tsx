...
export default function MediaPage() {
  ...
  const viewState: ViewState = isLoading ? "loading" : "success";

  return (
    <StateWrapper state={viewState} moduleName="Media Gallery">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center p-20 text-[var(--text-muted)] text-center border-2 border-dashed border-[var(--border)] rounded-[var(--radius-lg)]">
              ...
              <p className="text-xs mt-1">
                Run a query to generate publication-ready figures.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
              {artifacts.map((art) => (
                <div
                  key={art.artifact_id}
                  className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-lg)] overflow-hidden shadow-[var(--shadow-md)] flex flex-col group"
                >
