...
import { Search, Download, Database, Settings2, Plus, Loader2 } from "lucide-react";
        ...
        <div className="flex-1 flex overflow-hidden" style={{ background: "var(--bg-app)" }}>
            ...
            <div className="w-[240px] glass-sidebar border-r flex flex-col overflow-hidden">
                <div className="p-3 border-b" style={{ borderColor: "var(--border)" }}>
                    <h2 className="text-xs font-semibold text-[var(--text-primary)] mb-2">Filters</h2>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-4">
                    <FacetSection title="Entity Type">
                        {statsLoading ? (
                            ...
                            <div className="text-xs text-[var(--text-muted)] py-2 text-center">No types found</div>
                        ) : (
                            filteredTypes.map((t) => (
                                <button
                                    key={t.name}
                                ...
                                >
                                    <span>{t.name}</span>
                                    <span className="text-[10px] opacity-60 tabular-nums">{t.count.toLocaleString()}</span>
                                </button>
                            ))
                        )}
                    </FacetSection>
                    <FacetSection title="Provider Domains">
                        {[
                            { domain: "Proteomics", providers: ["UniProt", "InterPro", "STRING"], count: 3 },
                            { domain: "Genomics", providers: ["Ensembl", "GenomeAsia", "IndiGen", "IGVDB"], count: 4 },
                            { domain: "Chemistry", providers: ["ChEMBL", "PubChem", "DrugBank"], count: 3 },
                            { domain: "Literature", providers: ["PubMed", "Europe PMC"], count: 2 },
                            { domain: "Structures", providers: ["RCSB PDB", "AlphaFold"], count: 2 },
                            { domain: "Clinical", providers: ["ClinicalTrials.gov", "DisGeNET"], count: 2 },
                            { domain: "Pathways", providers: ["Reactome", "KEGG"], count: 2 },
                            { domain: "Patents", providers: ["PatentsView", "USPTO"], count: 2 },
                            { domain: "Targets", providers: ["OpenTargets"], count: 1 },
                        ].map(d => (
                            <div key={d.domain} className="flex items-center justify-between px-2 py-1 text-xs text-[var(--text-secondary)]">
                                <span>{d.domain}</span>
                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 tabular-nums">{d.count}</span>
                            </div>
                        ))}
                        <div className="text-[10px] text-[var(--text-muted)] px-2 pt-1 border-t mt-1" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="glass-panel border-b px-4 py-2.5 flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-[var(--text-primary)]">Entity Catalog</span>
                    {activeType && (
                        <span className="text-xs text-[var(--text-muted)]">
                            &mdash; {activeType}s
                            {searchResult && (
                                <span className="ml-1">({searchResult.total.toLocaleString()} total)</span>
                            )}
                        </span>
                    )}
                    <div className="ml-auto flex gap-2">
                        <button
                            onClick={() => {
                                ...
                                const blob = new Blob([JSON.stringify(searchResult.items, null, 2)], { type: "application/json" });
                                const url = URL.createObjectURL(blob);
                                const a = document.createElement("a"); a.href = url; a.download = `catalog_${activeType || "export"}.json`; a.click();
                                URL.revokeObjectURL(url);
                            }}
                            disabled={!searchResult?.items?.length}
                            className="flex items-center gap-1 px-2.5 py-1 text-xs rounded border hover:bg-gray-50 disabled:opacity-40"
                            style={{ borderColor: "var(--border)" }}
                        >
                    ...
                    {(searchLoading || searchFetching) ? (
                        <div className="glass-card rounded-xl p-8 h-full flex items-center justify-center">
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <p className="text-sm text-[var(--text-muted)]">Loading {activeType} data...</p>
                            </div>
                        </div>
                        ...
                        <div className="glass-card rounded-xl p-8 h-full flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <p className="text-sm text-slate-400 font-medium">No Results</p>
                ...
                <div className="glass-panel border-t px-4 py-2.5 flex items-center gap-4">
                    ...
                    <span className="text-[10px] text-[var(--text-muted)]">Collections:</span>
                    {statsLoading ? (
                        ...
                        <span className="text-[10px] text-[var(--text-muted)]">No stats available</span>
                    )}
                </div>
            </div>
        </div>
...
function FacetSection({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <div>
            <div className="text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1.5">{title}</div>
