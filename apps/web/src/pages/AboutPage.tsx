...
export default function AboutPage() {
  ...
  return (
    <StateWrapper
      ...
      emptyDescription="Unable to load system diagnostics."
      errorInfo={error ? { code: 'LOAD_ERROR', message: typeof error === 'string' ? error : 'Failed to load diagnostics' } : undefined}
      onRetry={refresh}
    >
    <div
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)] mt-0.5">
              System transparency, API health, model cards, and disclaimers
            </p>
          </div>
          <button
            onClick={refresh}
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border hover:bg-white transition-colors"
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="card rounded-xl p-4">
            <h2 className="text-xs font-semibold text-[var(--text-primary)] flex items-center gap-2 mb-3">
              <Server size={14} /> System Components
            </h2>
            <div className="space-y-2">
              {error ? (
                ...
                Object.entries(diag.components).map(([name, comp]) => (
                  <div
                    key={name}
...
FastAPI (:8000)
  ...
  └── SQLite (sessions, cache)`}</pre>
        ...
        <div className="card rounded-xl p-4 mb-6">
          <h2 className="text-xs font-semibold text-[var(--text-primary)] flex items-center gap-2 mb-3">
            <Globe size={14} /> External API Health
          </h2>
          <div className="grid grid-cols-3 gap-3">
            {diag ? (
              Object.entries(diag.connectors).map(([name, conn]) => (
                <div
                  key={name}
          ...
          <div className="card rounded-xl p-4">
            ...
            <div className="space-y-2 text-xs">
              ...
              >
                ...
                <span className="text-[var(--text-primary)] font-medium">
                  {diag ? Object.keys(diag.connectors).length : "—"}
                </span>
              </div>
          ...
          <div className="card rounded-xl p-4">
            ...
            <div className="space-y-2 text-xs text-[var(--text-muted)] leading-relaxed">
              <p>
                ...
                Drug Designer is a research aid. It is not a medical device,
              ...
              <p>
                ...
                AI-generated summaries are approximate and may contain errors.
              ...
              <p>
                ...
                Data is fetched from public APIs (UniProt, PubMed, RCSB, etc.)
                and may have different licensing terms.
              </p>
              <p>
                <strong className="text-[var(--text-secondary)]">
                  No Medical Advice.
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { icon: React.ReactNode; cls: string }> = {
    ...
    not_configured: { icon: <AlertCircle size={12} />, cls: "text-[var(--text-muted)]" },
  };
  const c = config[status] || config.not_configured;
  return (
    <span
      className={`flex items-center gap-1 text-[10px] font-medium ${c.cls}`}
    >
      {c.icon} {status}
    </span>
  );
  ...
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="h-6 bg-[var(--bg-inset)] rounded animate-pulse" />
      ))}
    </>
  );
}
