...
export default function DataPage() {
  ...
  const isLoading = keysQ.isLoading || connectorsQ.isLoading;
  const error = keysQ.error || connectorsQ.error;
  ...
  return (
    <StateWrapper
      ...
      emptyTitle="No Data"
      emptyDescription="No API keys or connectors found."
      errorInfo={error instanceof Error ? { code: 'FETCH_ERROR', message: error.message } : undefined}
      onRetry={() => { keysQ.refetch(); connectorsQ.refetch(); }}
    >
    <div
    ...
    >
      <div className="max-w-[1200px] mx-auto px-6 py-5">
        ...
        <p className="text-xs text-[var(--text-muted)] mb-5">
          Manage API keys, connectors, caches, and storage
        </p>
...
function APIKeysPanel() {
  ...
  const setKeyMut = useMutation({
    mutationFn: ({ service, key }: { service: string; key: string }) =>
      dataSetKeyAPI(service, key),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["dataKeys"] }),
  });
  ...
  return (
    <div className="card rounded-xl p-4">
      <h2 className="text-xs font-semibold text-[var(--text-primary)] flex items-center gap-2 mb-3">
      ...
      <div className="space-y-1 mb-3">
        {Object.entries(keys).map(([service, masked]) => (
          ...
          >
            ...
            <button
              onClick={() => delKeyMut.mutate(service)}
      ...
      <div className="flex gap-2">
        ...
        <div className="relative flex-1">
          <input
            ...
            placeholder="API Key"
            className="w-full px-2 py-1.5 text-xs rounded border pr-7"
            style={{ borderColor: "var(--border)" }}
          />
          <button
        ...
        <button
          onClick={() => {
            if (newService && newKey) {
              setKeyMut.mutate({ service: newService, key: newKey });
              setNewKey("");
            }
          }}
...
function ConnectorsPanel() {
  ...
  const connectorsQ = useQuery({
    ...
    queryFn: dataConnectorsAPI,
  });
  const toggleMut = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) =>
      dataToggleConnectorAPI(id, enabled),
        ...
        {connectors.map((c: ConnectorInfo) => (
          ...
          >
            <div className="flex items-center gap-2">
              <span
                className={`w-1.5 h-1.5 rounded-full ${c.enabled ? "bg-green-500" : "bg-slate-300"}`}
              />
              <span className="text-xs text-[var(--text-primary)]">
                {c.name}
              </span>
              {c.required && (
                <span className="text-[8px] px-1 py-0.5 rounded bg-[var(--bg-inset)] text-[var(--text-muted)]">
                  core
                </span>
              )}
            </div>
...
function CachePanel() {
  ...
  const clearMut = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["dataCache"] }),
  });
  ...
  return (
    <div className="card rounded-xl p-4">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded border text-red-600 hover:bg-red-50"
        style={{ borderColor: "var(--border)" }}
      >
        {clearMut.isPending ? (
