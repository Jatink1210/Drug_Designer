...
function statusBadge(status: string) {
  ...
  return map[status] || "bg-[var(--bg-inset)] text-[var(--text-primary)] border-gray-200";
}
...
export default function LogsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"logs" | "trace" | "recipe">(
    "logs",
  );
  ...
  const activeJobId =
    selectedJobId || (jobs && jobs.length > 0 ? jobs[0].job_id : null);
  ...
  function downloadLogs() {
    ...
    const lines = logsData.entries
      .map((e: LogEntry) => JSON.stringify(e))
      .join("\n");
  ...
  // ── Render ───────────────────────────────────────────────
  /* §A3.1: Page handles its own empty/loading states internally */
  const viewState: ViewState = isJobsLoading ? "loading" : "success";

  return (
    <StateWrapper state={viewState} moduleName="Execution Logs">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center text-xs text-[var(--text-muted)]">
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] uppercase font-bold border ${statusBadge(job.status)}`}
                    >
                      {job.status}
                    </span>
                  </div>
                    ...
                    >
                      ...
                      <span className="flex-1 text-[var(--text-primary)] break-words">
                        ...
                        {entry.status && entry.event === "step" && (
                          <span
                            className={`ml-2 text-[10px] px-1.5 py-0.5 rounded border uppercase font-bold ${statusBadge(entry.status)}`}
                          >
                            {entry.status}
                          </span>
                        )}
                      ...
                      <div className="flex-1 bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-lg)] shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] transition-all">
                        <div className="p-4 flex justify-between items-start">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-[var(--text-muted)] bg-[var(--border-light)] px-1.5 py-0.5 rounded">
                              {step.timestamp as string}
                            </span>
                          </div>
                          <span className="text-xs font-medium text-[var(--text-secondary)]">
                            {step.duration_ms as number}ms
                          </span>
                        </div>
                        {Boolean(step.details) &&
                          Object.keys(step.details as Record<string, unknown>)
                            .length > 0 && (
                              ...
                              <div className="text-xs text-[var(--text-secondary)] space-y-2">
                                {Object.entries(
                                  ...
                                  key === "prov_hash" ? (
                                    <div
                                      key={key}
                                      className="flex items-baseline gap-2 mt-3 pt-3 border-t border-[var(--border-light)]/50"
                                    >
                                      <span className="shrink-0 text-[var(--text-muted)] uppercase tracking-wider text-[10px] font-bold w-20">
                                        Prov Hash
                                      </span>
                                    ...
                                    <div key={key} className="flex gap-2">
                                      <span className="shrink-0 text-[var(--text-muted)] capitalize w-20">
                                        {key}:
                                      </span>
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-6 shadow-[var(--shadow-sm)]">
                  ...
                  <div className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-3 text-sm">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-xs uppercase font-bold border w-fit ${statusBadge(recipe.status)}`}
                    >
                      {recipe.status}
                    </span>
                    <span className="text-[var(--text-muted)] font-medium">
                    ...
                    <span className="text-[var(--text-primary)]">
                      {((recipe.duration_ms || 0) / 1000).toFixed(1)}s
                    </span>
                    <span className="text-[var(--text-muted)] font-medium">
                      Steps
                    </span>
                ...
                {recipe.environment && (
                  <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-6 shadow-[var(--shadow-sm)]">
                    ...
                    <div className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-2 text-sm">
                      {Object.entries(
                        recipe.environment as Record<string, string>,
                      ).map(([k, v]) => (
                ...
                {recipe.settings && (
                  <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-6 shadow-[var(--shadow-sm)]">
                    ...
                    <div className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-2 text-sm">
                      {Object.entries(
                        recipe.settings as Record<string, unknown>,
                      ).map(([k, v]) => (
                        <div key={k} className="contents">
                          <span className="text-[var(--text-muted)] font-medium capitalize">
                            {k.replace(/_/g, " ")}
                ...
                {recipe.connector_calls?.length > 0 && (
                  <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-6 shadow-[var(--shadow-sm)]">
                    ...
                    <div className="space-y-2">
                      {(
                        recipe.connector_calls as Array<Record<string, unknown>>
                      ).map((c, i: number) => (
                        <div
                          key={i}
                          className="flex items-center gap-4 text-sm"
                        >
                          <span className="w-6 text-right font-mono text-[var(--text-muted)]">
                            {c.step as number}
                          </span>
                          <span className="bg-[var(--border-light)] text-[var(--text-primary)] px-2 py-0.5 rounded font-mono text-xs">
                            {c.tool as string}
                          </span>
                          <span className="text-[var(--text-muted)] text-xs">
                            {c.duration_ms as number}ms
                          </span>
                        </div>
                      ))}
                    </div>
