...
export default function MiniGraphPreview({
  ...
  return (
    <div
      className="bg-[var(--bg-elevated)] rounded-lg border p-3"
    ...
    >
      ...
      <div className="flex flex-wrap gap-1">
        {nodes.slice(0, 12).map((n) => (
          <span
            key={n.id}
            className="px-1.5 py-0.5 rounded text-[10px] bg-[var(--bg-inset)] text-[var(--text-secondary)] truncate max-w-[120px]"
          >
            {n.label}
          </span>
        ))}
