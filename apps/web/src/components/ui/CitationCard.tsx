...
export default function CitationCard({ citation }: { citation: CitationRefDTO }) {
    const sourceColor = SOURCE_COLORS[citation.source] || "#6b7280";
    const typeInfo = TYPE_INDICATORS[citation.evidence_type] || TYPE_INDICATORS.neutral;
    ...
    return (
        ...
        >
            <div className="flex items-center gap-2 mb-1">
                ...
                {citation.year && (
                    <span className="text-[10px] text-[var(--text-muted)]">({citation.year})</span>
                )}
                <ExternalLink size={8} className="text-[var(--text-muted)] shrink-0 ml-auto" />
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] text-[var(--text-muted)] w-7 text-right">{confidencePct}%</span>
