...
export default function EvidenceBadge({
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-mono bg-[var(--bg-inset)] text-[var(--text-secondary)] hover:bg-[var(--bg-inset)] transition-colors"
    >
      {confidence != null && (
        <span
      ...
      <span className="truncate max-w-[80px]">
        {value.replace("PMID:", "").replace("NCT", "")}
      </span>
      {year && <span className="text-[var(--text-muted)]">({year})</span>}
      <ExternalLink size={8} />
    </a>
  );
}
