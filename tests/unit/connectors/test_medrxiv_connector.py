"""
Unit tests for MedRxiv Connector

Tests MedRxiv-specific functionality including:
...
class TestMedRxivConnector:
    """Test suite for MedRxiv connector"""
    
    def setup_method(self):
        """Setup test fixtures"""
        self.connector = MedRxivConnector()
    
    def test_initialization(self):
        """Test MedRxiv connector initialization"""
        assert self.connector.name == "MedRxiv"
    ...
    async def test_search_success(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({'collection': []}, {"source": "MedRxiv"})
    ...
    async def test_search_empty_results(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = (None, {"source": "MedRxiv"})
    ...
    async def test_fetch_by_id_success(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"id": "test_123", "name": "Test Entity"}, {"source": "MedRxiv"})
    ...
    async def test_caching_behavior(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({'collection': []}, {"source": "MedRxiv", "cache_hit": False})
            
            # First call
            result1 = await self.connector.search("test query")
            
            # Second call - should use cache
            mock_get.return_value = ({'collection': []}, {"source": "MedRxiv", "cache_hit": True})
    ...
    async def test_error_handling_invalid_response(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            ...
            mock_get.return_value = ({"invalid": "structure"}, {"source": "MedRxiv"})
    ...
    async def test_count_method(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"count": 42}, {"source": "MedRxiv"})
    ...
    async def test_extract_evidence(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"evidence": []}, {"source": "MedRxiv"})
