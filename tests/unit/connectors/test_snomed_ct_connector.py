"""
Unit tests for SNOMED CT Connector

Tests SNOMED CT-specific functionality including:
...
from connectors.snomed_ct import SNOMEDCTConnector


class TestSNOMEDCTConnector:
    """Test suite for SNOMED CT connector"""
    
    def setup_method(self):
        """Setup test fixtures"""
        self.connector = SNOMEDCTConnector()
    
    def test_initialization(self):
        """Test SNOMED CT connector initialization"""
        assert self.connector.name == "SNOMED CT"
    ...
    async def test_search_success(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({'items': []}, {"source": "SNOMED CT"})
    ...
    async def test_search_empty_results(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = (None, {"source": "SNOMED CT"})
    ...
    async def test_fetch_by_id_success(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"id": "test_123", "name": "Test Entity"}, {"source": "SNOMED CT"})
    ...
    async def test_caching_behavior(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({'items': []}, {"source": "SNOMED CT", "cache_hit": False})
            
            # First call
            result1 = await self.connector.search("test query")
            
            # Second call - should use cache
            mock_get.return_value = ({'items': []}, {"source": "SNOMED CT", "cache_hit": True})
    ...
    async def test_error_handling_invalid_response(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            ...
            mock_get.return_value = ({"invalid": "structure"}, {"source": "SNOMED CT"})
    ...
    async def test_count_method(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"count": 42}, {"source": "SNOMED CT"})
    ...
    async def test_extract_evidence(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"evidence": []}, {"source": "SNOMED CT"})
