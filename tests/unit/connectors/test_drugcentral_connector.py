"""
Unit tests for DrugCentral Connector

Tests DrugCentral-specific functionality including:
...
class TestDrugCentralConnector:
    """Test suite for DrugCentral connector"""
    
    def setup_method(self):
        """Setup test fixtures"""
        self.connector = DrugCentralConnector()
    
    def test_initialization(self):
        """Test DrugCentral connector initialization"""
        assert self.connector.name == "DrugCentral"
    ...
    async def test_search_success(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({'results': []}, {"source": "DrugCentral"})
    ...
    async def test_search_empty_results(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = (None, {"source": "DrugCentral"})
    ...
    async def test_fetch_by_id_success(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"id": "test_123", "name": "Test Entity"}, {"source": "DrugCentral"})
    ...
    async def test_caching_behavior(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({'results': []}, {"source": "DrugCentral", "cache_hit": False})
            
            # First call
            result1 = await self.connector.search("test query")
            
            # Second call - should use cache
            mock_get.return_value = ({'results': []}, {"source": "DrugCentral", "cache_hit": True})
    ...
    async def test_error_handling_invalid_response(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            ...
            mock_get.return_value = ({"invalid": "structure"}, {"source": "DrugCentral"})
    ...
    async def test_count_method(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"count": 42}, {"source": "DrugCentral"})
    ...
    async def test_extract_evidence(self):
        ...
        with patch.object(self.connector, '_cached_get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = ({"evidence": []}, {"source": "DrugCentral"})
